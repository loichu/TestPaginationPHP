...
function PasswordOrTextOrEmail($frmName, $fieldType, $fieldLabel, $fieldName, $isRequired = false, $isConfirm = false) {
    ...
    // Construit le champ
    //$field = "<tr>\n";
    $field = "<label for='$fieldName'>$fieldLabel :</label>\n";
    $field .= "\t<input type='$fieldType' name='$fieldName' value='$value' />\n";
    // Affiche les erreurs
    /*$field .= "\t<td><ul>\n";
    foreach ($errors as $error) {
        $field .= "\t\t<li><span class='erreur'>" . $error . "</span></li>\n";
...
function Submit($frmName, $submitName, $submitText){
    ...
    //$submit = "<tr>\n";
    //$submit .= "\t<th></th>\n";
    $submit = "<input type='submit' name='$submitName' value='$submitText' />\n";
    //$submit .= "</tr>\n";
    return $submit;
}
